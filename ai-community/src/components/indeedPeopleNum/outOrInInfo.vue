...
export default {
  ...
  created() {
    this.$axios({
      method: "get",
      url: "http://" + this.url + ":9000/sjwl/webapi/person/stream",
      headers: {
      ...
      if (res.data.code == 1000) {
        res.data.data.day.dateList.forEach((item, index) => {
          this.dayData.push({
            time: item * 1 + 1 + "",
            进入人数: res.data.data.day.enterCount[index] * 1,
            出去人数: res.data.data.day.outCount[index] * 1
          });
        });
        res.data.data.Week.dateList.forEach((item, index) => {
          this.WeekData.push({
            time: item,
            进入人数: res.data.data.day.enterCount[index] * 1+10,
            出去人数: res.data.data.day.outCount[index] * 1+6
          });
        });

        this.$nextTick(() => {
          switch ($(".title3 .selectedType").text()) {
        case "今日":
  ...
  mounted() {
    ...
    var _this = this;
    $(".title3 span").click(function() {
      $(".title3 span").removeClass("selectedType");
