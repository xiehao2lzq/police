...
export default {
  ...
  created () {
    this.$axios({
      method: "get",
      url: "http://" + this.url + ":9000/sjwl/webapi/person/stream/rule",
      headers: {
      ...
      if (res.data.code == 1000) {
        res.data.data.day.dateList.forEach((item, index) => {
          this.dayData.push({
            time: item * 1 + 1 + "",
            进入人员: res.data.data.day.enterCount[index] * 1,
            离开人员: res.data.data.day.outCount[index] * 1
          });
        });
